...
export const ListOfAreas: FC = () => {
    ...
    const {getListOfAreas} = useProceduresFunctions();
    useEffect(()=>{
        dispatch(getListOfAreas({CompanyId: 1, ManufactoryId: 1}));
    },[dispatch, getListOfAreas]);
    const handleGetClick = () => {
        if(isNaN(companyId) || isNaN(manufactoryId))
            return;
        dispatch(getListOfAreas({CompanyId: companyId, ManufactoryId: manufactoryId}));
    }
    if(!isLoaded)
        return <Loader />;
    return (
        <>
        <Body>
            <Title>LIST OF PRODUCTS OF AREAS MENU</Title>
            <List>
        ...
        <Body isDWidth={true}>
            <Title>LIST OF PRODUCTS OF AREAS TABLE</Title>
            <Table data={data}/>
        </Body>
        </>
    );
