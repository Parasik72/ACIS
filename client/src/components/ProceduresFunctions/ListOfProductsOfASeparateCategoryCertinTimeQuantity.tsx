...
export const ListOfProductsOfASeparateCategoryCertinTimeQuantity: FC = () => {
    ...
    const {getListOfProductsOfASeparateCategoryCertinTimeQuantity} = useProceduresFunctions();
    const formatYmd = (date: Date) => date.toISOString().slice(0, 10);
    useEffect(()=>{
        dispatch(getListOfProductsOfASeparateCategoryCertinTimeQuantity({CompanyId: 1, ManufactoryId: 1, AreaId: 1, DateFrom: '2002-01-01', DateTo: '2022-01-01'}));
    },[dispatch, getListOfProductsOfASeparateCategoryCertinTimeQuantity]);
    ...
    return (
        <>
        <Body>
            <Title>LIST OF PRODUCTS OF A SEPARATE CATEGORY CERTAIN TIME QUANTITY MENU</Title>
            <List>
        ...
        <Body isDWidth={true}>
            <Title>LIST OF PRODUCTS OF A SEPARATE CATEGORY CERTAIN TIME QUANTITY TABLE</Title>
            <Table data={data}/>
        </Body>
        </>
    );
