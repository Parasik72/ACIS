...
export const ListOfProductsCollectsNow: FC = () => {
    ...
    const {getListOfProductsCollectsNow} = useProceduresFunctions();
    useEffect(()=>{
        dispatch(getListOfProductsCollectsNow({CompanyId: 1, ManufactoryId: 1, AreaId: 1}));
    },[dispatch, getListOfProductsCollectsNow]);
    ...
    return (
        <>
        <Body>
            <Title>LIST OF PRODUCTS COLLECTS NOW MENU</Title>
            <List>
        ...
        <Body isDWidth={true}>
            <Title>LIST OF PRODUCTS COLLECTS NOW TABLE</Title>
            <Table data={data}/>
        </Body>
        </>
    );
